...
Until `claude plugin eval` is enabled for your account, run a case through `claude -p` in a project that has the skill installed. Allow the `Skill` tool, or the agent cannot load the skill and falls back to reading `SKILL.md` off disk. Isolate the run from your own skills, hooks, and MCP servers so the score reflects this skill alone:

```bash
echo '{"mcpServers":{}}' > /tmp/empty-mcp.json
claude -p "$(sed '1,/^---$/d' evals/keep-warm/prompt.md | sed '1,/^---$/d')" \
